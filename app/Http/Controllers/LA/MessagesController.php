    ...
    public function dtajax()
    {
        ...
        for ($i = 0; $i < count($data->data); $i++) {
            ...
            if ($this->show_action) {
                $output = '';
                if (Module::hasAccess("Messages", "edit")) {
                    $output .= '<a href="' . url(config('laraadmin.adminRoute') . '/messages/' . $data->data[$i][0] . '/edit') . '" class="btn btn-warning btn-sm" style="display:inline;"><i class="fa fa-edit"></i></a>';
                }

                if (Module::hasAccess("Messages", "delete")) {
                    $output .= Form::open(['route' => [config('laraadmin.adminRoute') . '.messages.destroy', $data->data[$i][0]], 'method' => 'delete', 'style' => 'display:inline']);
                    $output .= ' <button class="btn btn-dange btn-sm" type="submit"><i class="fa fa-times"></i></button>';
                    $output .= Form::close();
                }
