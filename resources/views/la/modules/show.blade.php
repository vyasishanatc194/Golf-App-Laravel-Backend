	...
	<div class="row mb-5">
		<div class="col-md-12">
			<div class="card">	
				...
				<a data-toggle="modal" data-target="#AddFieldModal" class="btn btn-success btn-sm  pull-right btn-add-field" >Add Field</a>				
					<ul id="module-tabs" data-toggle="ajax-tab" class="nav nav-pills card-header-pills" role="tablist">
						<li class="nav-item"><a class="nav-link" href="{{ url(config('laraadmin.adminRoute') . '/modules') }}" data-toggle="tooltip" data-placement="right" title="Back to Modules"> <i class="fa fa-chevron-left"></i>&nbsp;</a></li>
						
						<li class="nav-item" id="fields">
							<a id="tab_fields" role="tab" data-toggle="tab" class="nav-link active show" href="#fields" data-target="#tab-info"><i class="fa fa-bars"></i> Module Fields</a>
						</li>
						
						<li class="nav-item" id="access">
							<a id="tab_access" role="tab" data-toggle="tab"  class="nav-link " href="#access" data-target="#tab-access"><i class="fa fa-key"></i> Access</a>
						</li>
						
						<li class="nav-item" id="sort">
							<a id="tab_sort" role="tab" data-toggle="tab"  class="nav-link" href="#sort" data-target="#tab-sort"><i class="fa fa-sort"></i> Sort</a>
						</li>
					</ul>
				</div>
				<div class="card-body">
					<div class="tab-content">
						<div role="tabpanel" class="tab-pane fade in active show" id="tab-info">
							<div class="tab-content">
								<div class="panel">
												...
												@foreach ($module->fields as $field)
													<tr>
														...
														<td>
																...
																<?php $arr = json_decode($field['popup_vals']);?>
																@if($arr )
																@foreach($arr as $popup_val)
														...
														<td>
															<a href="{{ url(config('laraadmin.adminRoute') . '/module_fields/'.$field['id'].'/edit') }}" class="btn btn-edit-field btn-warning btn-sm"  id="edit_{{ $field['colname'] }}"><i class="fa fa-edit"></i></a>
															<a href="{{ url(config('laraadmin.adminRoute') . '/module_fields/'.$field['id'].'/delete') }}" class="btn btn-edit-field btn-danger btn-sm"  id="delete_{{ $field['colname'] }}"><i class="fa fa-trash"></i></a>
															@if($field['colname'] != $module->view_col)
																<a href="{{ url(config('laraadmin.adminRoute') . '/modules/'.$module->id.'/set_view_col/'.$field['colname']) }}" class="btn btn-edit-field btn-success btn-sm" id="view_col_{{ $field['colname'] }}"><i class="fa fa-eye"></i></a>
															@endif
														</td>
													</tr>
												@endforeach
						...
						<div role="tabpanel" class="tab-pane fade in" id="tab-access">
							<div>
								<span class="pull-left"><h4>Module Access for Roles</h4></span>
								<span class="pull-right">
								<i class="fa fa-circle"></i> Invisible 
								<i class="fa fa-circle  text-warning"></i> Read-Only 
								<i class="fa fa-circle text-success"></i> Write
								</span>
							</div>
													...
													<tr>
														@foreach ($fields as $field)
															<td>
																<div class=" row">
																	<div class="col-md-4">
																		<input type="text" name="{{ $field['colname'] }}_{{ $role->id }}" value="{{ $role->fields[$field['id']]['access'] }}" data-slider-value="{{ $role->fields[$field['id']]['access'] }}" class="slider form-control" data-slider-min="0" data-slider-max="2" data-slider-step="1" data-slider-orientation="horizontal"  data-slider-id="{{ $field['colname'] }}_{{ $role->id }}">
																	</div> 
																	<div class="col-md-8">
																	{{ $field['label'] }}
																	</div>
						...
						<div role="tabpanel" class="tab-pane fade in" id="tab-sort">
							<div class="row">
...
.table-access input[type="checkbox"]{vertical-align:text-top;}
...
$(function () {
		...
		$(".slider.slider-horizontal").each(function(index) {
			...
			switch (value) {
				case '0':
					$(this).find('.slider-handle').removeClass("orange");
					$(this).find('.slider-handle').removeClass("green");
					$(this).find('.slider-handle').addClass("gray");
					break;
				case '1':
					$(this).find('.slider-handle').removeClass("gray");
					$(this).find('.slider-handle').removeClass("green");
					$(this).find('.slider-handle').addClass("orange");
					break;
				case '2':
					$(this).find('.slider-handle').removeClass("gray");
					$(this).find('.slider-handle').removeClass("orange");
					$(this).find('.slider-handle').addClass("green");
					break;
			}
		});
