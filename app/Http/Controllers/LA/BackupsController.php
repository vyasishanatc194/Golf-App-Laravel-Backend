...
class BackupsController extends Controller
{
	...
	public function dtajax()
	{
		...
		for($i=0; $i < count($data->data); $i++) {
			...
			if($this->show_action) {
				$output = '';
				$output .= '<a href="'.url(config('laraadmin.adminRoute') . '/downloadBackup/'.$data->data[$i][0]).'" class="btn btn-warning btn-sm" style="display:inline;"><i class="fa fa-download"></i></a>';
