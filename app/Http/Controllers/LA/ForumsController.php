...
//		}
	...
	public function dtajax()
	{
		...
		$forumThread = \App\Models\Thread::leftJoin('users','users.id','=','forum_threads.author_id')
		->leftJoin('forum_categories', 'forum_categories.id','=', 'forum_threads.category_id')
		->select('forum_threads.id', 'forum_threads.title', 'forum_threads.content', 'forum_categories.title as categoryname', 'forum_threads.total_likes', 'forum_threads.total_comments', 'forum_threads.total_posts');

		// $values = DB::table('forums')->select($this->listing_cols)->whereNull('deleted_at');
		return Datatables::of($forumThread)->make(true);
		// $data = $out->getData();
		...
		// 	$output = '';
		// 	if(Module::hasAccess("Forums", "edit")) {
		// 		$output .= '<a href="'.url(config('laraadmin.adminRoute') . '/forums/'.$data->data[$i][0].'/edit').'" class="btn btn-warning btn-sm" style="display:inline;"><i class="fa fa-edit"></i></a>';
		// 	}
